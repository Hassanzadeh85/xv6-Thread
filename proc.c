...
// shares the parent's address with thread
  thread->pgdir = curproc->pgdir;
  
//saving important information from the parent process
  thread->sz = curproc->sz; 
  thread->parent = curproc; 
  
// Copy the values of reegisters from the current process
// to the registers of the new thread
  *thread->tf = *curproc->tf;

// stack parameters
  uint user_stack[2];
  user_stack[0] = 0xffffffff; // where stack once it starts
  user_stack[1] = (uint) arg; // the clone argument passed to
  
// set top of the stack to the allocated page
  uint stack_top = (uint) stack + PGSIZE;
  
// subtract 8 bytes(2*32bit) from the stack_top to
// make space for save two values 
  stack_top -= 8;
  
// copy user stack values to memory
  if (copyout(thread->pgdir, stack_top, user_stack, 8) < 0)
  {
  return -1;
  }

// set stack base and stack topc
  
  thread->tf->ebp = (uint) stack_top;
  thread->tf->esp = (uint) stack_top;
  
// set instruction pointer to address of fcn
  thread->tf->eip = (uint) fcn;
  
// Clear eax so that clone returns 0 in the child thread.
  thread->tf->eax = 0;

// copying all open files
...
//-------------------------------------------------------------------
// join system call definition to wait for exit all child threads. 
int join(void)
{
  struct proc *p;
  int haveThreads, pid;
  ...
  for(;;) 
  {
// search ptable for find exited children.

      haveThreads = 0;
      for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
      {

// Check if thread "p" is a child thread
